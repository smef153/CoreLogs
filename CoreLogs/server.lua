...
RegisterNetEvent("CoreLog")
AddEventHandler("CoreLog", function()
...
function renterdiscord(message)
	local content = {
  	...
  	PerformHttpRequest( --[[webhook here--]]" , function(err, text, headers) end, 'POST', json.encode({username = name, embeds = content}), { ['Content-Type'] = 'application/json' })
end
